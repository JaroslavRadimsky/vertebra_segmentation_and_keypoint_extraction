...
from Src.Atlas.training.common import get_test_transformation, get_test_transformation_with_heqv # tuto transformaci budu pouzivat
from Src.project_paths import ATLAS_DATA_DIR, ATLAS_FOLDS_DIR, RESULTS_DIR, SEGMENTATION_CHECKPOINTS_DIR

import torch
from torch.utils.data import  DataLoader

#---------------general configuration-------------------
MAX_FILES = 10_000 # pocet souboru pro testovani
...
config["TRAIN"] = False
config["LOAD_TRAIN_MODEL"] = SEGMENTATION_CHECKPOINTS_DIR / config["NAME"]
   ...
   for fold_id, fold in enumerate(folds[START_FOLD:START_FOLD+1], start=START_FOLD):
      ...
      if config["LOAD_TRAIN_MODEL"] is not None:  # Pokud je zadáno jméno modelu, tak se načte
       # load stored model for testing
         ...
         result_dir = RESULTS_DIR / "Atlas" / "multiclass"
         result_dir.mkdir(parents=True, exist_ok=True)
         result_df_fold_val.to_csv(result_dir / f"{config['NAME']}-fold-{fold_id}-val-xxx.csv", index=False)
         result_df_fold_train.to_csv(result_dir / f"{config['NAME']}-fold-{fold_id}-train-xxx.csv", index=False)
         print(f"---------- fold {fold_id} --------")
